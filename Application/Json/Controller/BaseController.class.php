...
class BaseController extends Controller {
	...
	public static function doSuccess($list=array()){
		echo json_encode(array('code' => '200', 'msg' => '成功', 'data' => $list));exit;
	}

	public static function doFailed($msg='',$code='-1'){
		echo json_encode(array('code' => $code, 'msg' => $msg, 'data' => array()));exit;
	}
...
//         		$message=iconv('utf-8','gb2312',$message);
	...
	public function auth() {
		...
		try{
			$user_name = Util::getSaveString ( "user_name" );
...
//		$token = I('post.access_token');
	...
	public static function getUserId($user_name){
		$model = M('member');
		$list = $model->field('id')->where("user_name = '$user_name'")->find();
		if($list){
			return $list['id'];
		}else{
			self::doFailed('用户不存在');

		       ...
		       <table class="table table-striped table-hover table-bordered">
					 ...
					 <?php if(!empty($Child)): if(is_array($Child)): $i = 0; $__LIST__ = $Child;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							  ...
							  <td>
							   ...
							   <td>
								   <?php if($vo["is_pay"] == 1): ?><a class="btn btn-info btn-sm view-result" href="javascript:void(0)" iid="<?php echo ($vo["article_id"]); ?>"> 查看</a><?php endif; ?>
								   <?php if($vo["pay_num"] != 0): ?><a style="margin-left:20px;" class="btn btn-info btn-sm get-info" href="javascript:void(0)" iid="<?php echo ($vo["article_id"]); ?>"> 付费次数</a><?php endif; ?>
									<!--<a style="margin-left:20px;" class="btn btn-info btn-sm get-result" href="javascript:void(0)" iid="<?php echo ($vo["article_id"]); ?>"> 被点击次数</a>-->
							   </td>
...
<?php if(is_array($Child)): $i = 0; $__LIST__ = $Child;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="modal fade" id="myInfo_<?php echo ($vo["article_id"]); ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			...
			<div class="row">
			   <div class="col-md-10 col-md-offset-1">
				   <table class="table table-bordered">
					   ...
					   <?php if(is_array($vo["pay_list"])): $i = 0; $__LIST__ = $vo["pay_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($list["user_name"]); ?></td><td><?php echo ($list["pay_coin"]); ?></td><td><?php echo ($list["pay_time"]); ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
				   </table>
			   </div>
			</div>
		  </div>
...
<script type="text/javascript" src="/Public/Home/js/habit.js"></script>
